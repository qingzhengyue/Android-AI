        ...
        return when {
                ...
                if (isWorkspaceEmpty) {
                    ...
                    val adviceSection = when {
                        ...
                        !hasLoop -> "【修正建议】: \n" +
                            ...
                            "② 将它吸附包裹在蓝色【移动】积木的外部；\n" +
                            "③ 在蓝色【运动】分类中拖出【碰到边缘就反弹】放入循环底部，点击绿旗测试！✨"
                        else -> "【修正建议】: \n" +
                            "① 可以尝试在紫色【外观】分类中拖出【下一个造型】，放入循环中实现奔跑动画；\n" +
                            "② 在深橙色【变量】分类中建立变量（如【得分】），让作品更有游戏趣味性！🚀"
                    }
            ...
            prompt.contains("创意引导") || prompt.contains("创作主题") || prompt.contains("自由拓展") || q.contains("创意引导") || q.contains("创意拓展") -> {
                val theme = when {
                    ...
                    cleanQuestion.contains("创意探索:") -> cleanQuestion.substringAfter("创意探索:").trim()
                    cleanQuestion.isNotBlank() && !cleanQuestion.contains("创意引导") -> cleanQuestion
                    else -> "格斗与动作对战"
                }
                ...
                when {
                    t.contains("格斗") || t.contains("对战") || t.contains("拳击") || t.contains("打斗") || t.contains("战斗") || t.contains("pk") -> {
                    ...
                    t.contains("太空") || t.contains("宇宙") || t.contains("火箭") || t.contains("星球") || t.contains("飞行") -> {
                        "🎨【 智能精灵姐姐 · 【$theme】3大星际探险魔法方案 】🚀\n\n" +
                        "太棒啦！【$theme】主题充满未来科技感，精灵姐姐教你用积木搭出属于你的星际大片：\n\n" +
                        "🌟 **魔法方案 1：【火箭推进与惯性巡航】**\n" +
                        "• **玩法**：按住空格键喷射火焰加速，松开后在无重力太空中滑行！\n" +
                        "• **拼法**：在【重复执行】里放入【如果 <按下空格键?> 那么 [将 (速度) 增加 1, 换成 (喷火造型)] 否则 [将 (速度) 增加 -0.5]】。\n\n" +
                        "🌟 **魔法方案 2：【克隆陨石雨与激光炮防御】**\n" +
                        "• **玩法**：太空上方源源不断掉落克隆陨石，按下鼠标发射激光打碎陨石！\n" +
                        "• **拼法**：克隆体启动时【移到 x: (在 -200 到 200 间随机数) y: 180】，在循环中【将 y 坐标增加 -5】！\n\n" +
                        "🌟 **魔法方案 3：【虫洞穿越与能量护盾】**\n" +
                        "• **玩法**：飞入虫洞瞬间随机传送到新的神秘星系，开启金色光晕护盾！\n" +
                        "• **拼法**：紫色【外观】拖出【将 颜色 特效增加 25】，黄色【控制】拖出【如果 <碰到 虫洞?> 那么 [移到 随机位置]】！✨"
                    }
                    t.contains("迷宫") || t.contains("探险") || t.contains("逃生") || t.contains("闯关") -> {
                        ...
                        "🌟 **魔法方案 3：【幽灵守卫巡逻追逐】**\n" +
                        "• **玩法**：迷宫中有巡逻的小幽灵，一旦被小幽灵碰到就会重回起点！\n" +
                        "• **拼法**：小猫脚本中加入【如果 <碰到 [小幽灵]?> 那么 [移到 x: -200 y: -150 (起点坐标)]】！✨"
                    }
                    else -> {
                        "🎨【 智能精灵姐姐 · 【$theme】3大专属创意魔法方案 】🚀\n\n" +
                        "哇！围绕【$theme】这个超酷的主题，精灵姐姐为你准备了 3 个充满童趣的创意编程方案：\n\n" +
                        "🌟 **魔法方案 1：【主题核心玩法与角色互动】**\n" +
                        "• **玩法**：让你的核心角色根据【$theme】情境展开生动动作，配合键盘互动与造型变换！\n" +
                        "• **拼法**：黄色【当按下 方向键】 ➔ 蓝色【面向 90 方向】 ➔ 蓝色【移动 15 步】 ➔ 紫色【下一个造型】！\n\n" +
                        "🌟 **魔法方案 2：【积分与荣誉关卡系统】**\n" +
                        "• **玩法**：在【$theme】过程中达成目标（如搜集物品、答对题目），积分增加并触发金光庆祝特效！\n" +
                        "• **拼法**：在深橙色【变量】新建变量【得分】；在控制分类拖出【如果 <碰到 目标?> 那么 [将 得分 增加 1, 播放声音 胜利]】。\n\n" +
                        "🌟 **魔法方案 3：【动态特效与通关胜利仪式】**\n" +
                        "• **玩法**：当通关时，全屏燃放烟花，所有角色一起跳舞欢呼！\n" +
                        "• **拼法**：黄色【广播 [胜利大狂欢]】 ➔ 各角色【当接收到 [胜利大狂欢] ➔ 重复执行 10 次 [右转 36 度, 等待 0.1 秒]】！✨"
                    }
                }
            }
            ...
            prompt.contains("知识点讲解") || prompt.contains("考点讲解") || prompt.contains("核心考点") || q.contains("考点讲解") || q.contains("知识点讲解") -> {
                val topic = when {
                    ...
                    cleanQuestion.contains("考点解析:") -> cleanQuestion.substringAfter("考点解析:").trim()
                    cleanQuestion.isNotBlank() && !cleanQuestion.contains("考点讲解") && !cleanQuestion.contains("知识点讲解") -> cleanQuestion
                    else -> "自制积木与函数"
                }
                ...
                when {
                    ...
                    t.contains("变量") || t.contains("数据") || t.contains("得分") -> {
                    ...
                    t.contains("循环") || t.contains("重复") -> {
                        ...
                        "1. 💡 **什么是【$topic】？**\n" +
                        "   • 在少儿编程中，【$topic】是构建互动程序和游戏必不可少的关键积木工具！\n" +
                        "   • 它可以帮助角色更精确地感知外界输入，做出聪明的逻辑反应。\n\n" +
                        "2. 🧩 **动手拼搭指引**：\n" +
                        "   • ① 在左侧分类栏找到与【$topic】对应的彩色分类标签；\n" +
                        "   • ② 拖拽相关积木吸附在【当 🟢 被点击】或循环结构的内部；\n" +
                        "   • ③ 结合黄色【控制】与紫色【外观】实现生动的舞台互动反馈！\n\n" +
                        "3. 🌟 **学习小贴士**：多动手点击绿旗调试运行，看看角色在舞台上的即时变化吧！🚀"
                    }
                }
            }
