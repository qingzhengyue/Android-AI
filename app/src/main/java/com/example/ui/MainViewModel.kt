        ...
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            ...
            if (!countOk) {
                ...
                if (!grammarCorrect) {
                    ...
                    val modeInstruction = if (mode == "专家") {
                        """
                        【当前辅导模式】：🎓【专家深度启发模式】
                        - 适用对象：中高年级（4-6年级）或希望探究原理、进阶算法的小创客。
                        - 指导策略：
                          1. 深度剖析 Scratch 底层逻辑与运行原理（如笛卡尔坐标系统边界判定、向量反射角计算、克隆体生命周期与时序机制等）；
                          2. 提供【方案一：基础积木解法】与【方案二：自制积木/变量算法进阶解法】；
                          3. 提出 1 个启发式进阶思考题，引导学生培养计算思维与自主探究能力。
                        """.trimIndent()
                    } else {
                        """
                        【当前辅导模式】：⚡️【快速极速速答模式】
                        - 适用对象：低年级（1-3年级）或初学者，需要立竿见影解决积木卡壳或报错。
                        - 指导策略：
                          1. 极简 3 步法直奔主题：① 找准分类颜色 ➔ ② 拖出积木名称 ➔ ③ 拼装在何处；
                          2. 零理论门槛，语言极其通俗生动，多用生活比喻；
                          3. 附带 1 个关键避坑提示（例如旋转方式设为左右翻转等）。
                        """.trimIndent()
                    }
                    ...
                    val prompt = """
                        ...
                        请结合小朋友的问题、当前选择的辅导模式以及积木代码，给出量身定制的最佳辅导回复！
                    """.trimIndent()
